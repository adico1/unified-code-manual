...
1. exactly one pinned בלי_מה root, two interface registries, two family bases,
   13 direct מה seeds, and 51 content-addressed derived מה programs;
2. 64 distinct generated application sources;
...
11. rejection of altered, floating, cyclic, conflicting, and escaping base
    authority;
12. zero stored leaf ASTs and 64 build-time-generated ASTs.
...
20. all 32 Todo profiles prove exact state after atomic persistence and
    restart.
21. vocabulary derived from all 64 applications has zero hits in compiler
    sources.
22. every calculator acceptance expression exposed by the editable GUI is
    ...
    result before lazy launch.
23. the complete verification operation stays within the five-second
    post-tool-boot budget.
...
applications = 64
acceptance = 163/163
isolated copied applications = 64/64
deterministic = PASS
runtime seed access = 0
manual application code = 0
manual application tests = 0
compiler application-vocabulary hits = 0/326
...
generated Key callback wiring checks = 911/911
application-owned GUI and acceptance self-tests = 941/941
self-test applications closed = 64/64
runtime Key-registry access = 0
stored leaf ASTs = 0
build-time-generated ASTs = 64/64
registered build-time stampers = 6
derived transitions = 911
...
- The concise declarations generate the Python AST; it is not copied into an
  application seed.
- Thirteen applications are direct leaves. Fifty-one use a pinned prototype
...
- This proves 64 bounded profiles, not every calculator or Todo application,
